...
- **Agent**: ADA Agent — the AI Executive Assistant system that processes user instructions and generates outputs.
...
- **Audit Log**: A tamper-evident, persistent record of Agent actions, approval decisions, and data access events. Stored in SQLite and survives server restarts.
...
- **Meeting Notes**: Plain-text or Markdown files (.txt, .md) attached by the User in the chat interface to provide context for draft generation.
...
6. IF the Agent cannot generate a Draft due to insufficient instructions or unreadable context, THEN THE Agent SHALL respond conversationally asking the User for the missing information rather than producing an incomplete or placeholder Draft.
7. WHEN the Agent cannot extract usable content from attached meeting notes, THE Agent SHALL respond with a clarification request and SHALL NOT generate a draft email addressed to the recipient asking them for the information.
...
1. WHEN the User requests retrieval of related emails, THE Agent SHALL present a permission prompt specifying the scope of Gmail access before accessing any Gmail data.
...
### Requirement 4: Meeting Notes Attachment

**User Story:** As a manager, I want to attach meeting notes when drafting follow-up emails, so that my communications accurately reflect decisions and action items.

#### Acceptance Criteria

1. THE Agent SHALL allow the User to attach plain-text (.txt) or Markdown (.md) files as meeting notes via the chat interface.
2. THE Agent SHALL reject binary file formats (PDF, DOCX, etc.) and inform the User to paste the content as plain text instead.
3. WHEN meeting notes are attached, THE Agent SHALL sanitise the content through the Prompt Builder & Sanitiser before passing it to the LLM.
4. WHEN meeting notes are used to generate a Draft, THE Agent SHALL include them in the Context Summary.
5. IF the attached meeting notes are unreadable or contain no extractable content, THE Agent SHALL respond conversationally asking the User to provide the information in a different format.
6. WHEN a Prompt Injection attempt is detected in attached meeting notes, THE Agent SHALL flag the event in the Audit Log and notify the User.
...
1. WHEN the User initiates a send action on a Draft, THE Agent SHALL display an Approval Gate showing the recipient, subject, full email body, and the Context Summary.
2. WHILE the User has not confirmed the Approval Gate, THE Agent SHALL not transmit the email.
3. WHEN the User confirms the Approval Gate, THE Agent SHALL send the email (or simulate the send if Gmail is not connected) and record the action in the Audit Log.
...
1. WHEN the User requests to schedule a Draft using natural language (e.g. "schedule for tomorrow at 9am"), THE Agent SHALL first generate the Draft via LLM, then parse the send time from the instruction, then display an Approval Gate.
2. THE Approval Gate SHALL show the email content, recipient, body, and the proposed send time before scheduling.
3. WHILE the User has not confirmed the scheduling Approval Gate, THE Agent SHALL not register the Scheduled Draft.
4. WHEN the User confirms the scheduling Approval Gate, THE Agent SHALL register the Scheduled Draft and record the action in the Audit Log.
5. WHEN a Scheduled Draft is registered, THE Agent SHALL allow the User to cancel the Scheduled Draft at any time before the scheduled send time.
6. WHEN the scheduled send time arrives, THE Agent SHALL present an active confirmation prompt to the User requiring explicit confirmation before the email is transmitted.
7. IF the User does not confirm within a configurable timeout period, THE Agent SHALL hold the email without sending and notify the User.
...
6. THE LLM system prompt SHALL instruct the model to respond with a `CLARIFICATION_NEEDED:` prefix when it cannot extract usable content from the data context, rather than generating a placeholder draft.
...
3. THE Agent SHALL display the current set of active permissions to the User on request.
...
5. WHEN the User reviews the Audit Log, THE Agent SHALL display entries in reverse chronological order with filtering by event type and session ID.
6. THE Audit Log SHALL be persisted to a SQLite database and survive server restarts.
...
5. THE Agent SHALL communicate with all external data sources over encrypted connections using industry-standard protocols.
...
### Requirement 15: Persistent User and Session Storage

**User Story:** As a user, I want my login credentials and activity history to be saved across server restarts, so that I don't lose my data when the server is restarted.

#### Acceptance Criteria

1. THE Agent SHALL persist user accounts (email address and hashed password) to Google Cloud Firestore on registration.
2. THE Agent SHALL persist all session records (session ID, user ID, created time, last active time, expiry) to Firestore.
3. WHEN a User logs in with a registered email and correct password, THE Agent SHALL authenticate them against the stored hash.
4. WHEN a User logs in with a registered email and incorrect password, THE Agent SHALL reject the login and log an AUTH_FAILED event.
5. User passwords SHALL be stored as SHA-256 hashes and never in plain text.
6. THE database credentials and token files SHALL be excluded from version control.
7. THE system SHALL be designed to allow migration between database backends by replacing only the database layer (`src/db/database.py`) without changes to any other component.

---

### Requirement 16: User Registration and Account Management

**User Story:** As a new user, I want to create my own account with a unique email address and password, so that my data is private and separate from other users.

#### Acceptance Criteria

1. THE Agent SHALL provide a Sign Up form allowing new users to register with an email address and password.
2. THE Agent SHALL validate that the email address is unique — if an account already exists for that email, THE Agent SHALL return an error and not create a duplicate account.
3. THE Agent SHALL require passwords to be at least 6 characters long.
4. THE Agent SHALL require the user to confirm their password during registration and reject mismatched entries.
5. WHEN registration succeeds, THE Agent SHALL automatically create a session and log the user in.
6. THE Agent SHALL provide a Sign In form for existing users to authenticate with their email and password.
7. WHEN a user attempts to sign in with an email that has no registered account, THE Agent SHALL return an error — no auto-creation of accounts on login.
8. All user data (audit log, tone profiles, scheduled drafts, sessions) SHALL be scoped to the authenticated user's email address.
9. THE Agent SHALL display Sign In and Sign Up as separate tabs on the authentication screen.
